 ...
 * So we add a transient which will be handled by the `patch_admin_redirect_to_pixcare_install_once` action
 */
function patch_force_redirect_to_pixcare_install_once() {
	...
	if ( empty( $plugin_version ) ) {
		set_transient( '_patch_activation_redirect', 1 );
	}
}
add_action( 'after_switch_theme', 'patch_force_redirect_to_pixcare_install_once' );

/**
 *  * Redirect the admin to the pixcare install page once only if the plugin is missing
 */
function patch_admin_redirect_to_pixcare_install_once() {
	if ( ! get_transient( '_patch_activation_redirect' ) ) {
		return;
	}
	delete_transient( '_patch_activation_redirect' );
...
add_action( 'admin_init', 'patch_admin_redirect_to_pixcare_install_once' );

function patch_add_pixcare_install_page() {
	add_submenu_page( null, '', '', 'manage_options', 'pixelgrade_care-install', null );
}
add_action( 'admin_menu', 'patch_add_pixcare_install_page' );

function patch_pixcare_install_page() {
	...
	<head>
		...
		<title><?php esc_html_e( 'Pixelgrade Care &rsaquo; Installer', 'patch' ); ?></title>
			...
			<div class="stepper">
				<div class="stepper__content">
					...
					<div class="section section--informative">
						...
						<h2 class="section__title u-text-center"><?php esc_html_e( 'One more lap to go..', 'patch' ); ?></h2>
						<div class="section__content">
							<?php
							echo sprintf(
								'%s <strong>%s</strong> %s',
								esc_html__( 'Start your digital journey with our', 'patch' ),
								esc_html__( 'Pixelgrade Care®', 'patch' ),
								esc_html__( 'on board. It\'s the core of your experience with our WordPress theme, and we bet you are going to love it from day one. You’re in great hands, no doubt about that!', 'patch' )
							); ?>
						</div>
						<div class="plugins">
							<div class="plugin  box  box--neutral  box--plugin-missing  box--plugin-invalidated">
								<div class="box__body">
									<h5 class="box__title"><?php esc_html_e( 'Pixelgrade Care®', 'patch' ) ?></h5>
									<p class="box__text"><?php esc_html_e( 'Installing ...', 'patch' ) ?></p>
								</div>
							</div>
						</div>
					</div>
					...
					<a class="btn  btn--text  btn--return-to-dashboard"
					   href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'patch' ) ?></a>
				</div>
			</div>
		</div>
	</div>
		...
		(function ($) {
			$(document).ready(function () {
				...
				wp.ajax.send({type: 'GET'}).always(function (res) {
					var success = res.indexOf('<p><?php esc_html_e( 'Plugin installed successfully.', 'patch' ); ?></p>');
					var already_exists = res.indexOf('<p><?php esc_html_e( 'Plugin destination folder already exists.', 'patch' ); ?></p>');
					...
					if (success !== -1) {
						...
						$status.text('<?php esc_html_e( 'Bummer! Something went wrong. I’m sorry but you will need to install the plugin manually. Redirecting you to the appropriate page…', 'patch' ); ?>');
...
add_action( 'admin_init', 'patch_pixcare_install_page' );
